...
2. Fully silent behavior requires **agent-first startup**: post an invisible Discord message (just the marker embed, no content, no attachment), create thread, enqueue prompt via SDK, create IPC `start_thread_listener` request, and keep the starter message so Discord thread history stays valid. The bot's IPC polling picks up the request within 200ms and streams the response directly to the thread.
...
5. On this host, making the fork default required replacing `/usr/bin/otto` with a symlink to `/data/projects/bridge/cli/bin.js` and restarting the bot.
...
9. **Bot's `channelThreadCreate` handler interaction**: For invisible starter messages, `channelThreadCreate` sees `start: true` but prompt extraction can still be empty (`No prompt found in starter message`). Session startup should therefore rely on IPC `start_thread_listener` as source of truth. Keep the starter message to avoid Discord's `couldn't load the first message` thread UI error.
10. **IPC polling interval is 200ms** (`ipc-polling.ts`). The IPC request is picked up almost immediately after the thread is created. The session prompt has already been queued via `session.promptAsync()` before the IPC request is created, so no race condition.

## Verification checklist used

- `cd cli && npx tsc --noEmit`
- `cd cli && npx vitest run src/system-message.test.ts src/task-schedule.test.ts`
- Runtime smoke: `otto send --silent-prompt ...` and confirm thread creation + IPC listener startup in `~/.otto/otto.log`.
- For scheduled tasks: schedule a test task with `otto send --send-at <time> --silent-prompt ...` and verify the Discord thread shows no user message or attachment.
