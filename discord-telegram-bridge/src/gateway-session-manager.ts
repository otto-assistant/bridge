...
type GatewayAuthorizeCallback = (context: {
  kind: string
  token: string
}) => Promise<{ allow: boolean; clientId?: string; authorizedTeamIds?: string[] }>
...
export class GatewaySessionManager {
  ...
  private readonly authorize?: GatewayAuthorizeCallback
  ...
  constructor({
    ...
    authorize?: GatewayAuthorizeCallback
  }) {
  ...
  private async authenticateGatewayIdentify(token: string): Promise<{
    ...
    if (this.authorize) {
      ...
      if (result.authorizedTeamIds && !result.authorizedTeamIds.includes(this.workspaceId)) {
        return { allow: false }
      }
      return result
    }
