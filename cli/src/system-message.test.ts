...
describe('system-message', () => {
  ...
  test('keeps the system prompt session-scoped', () => {
    ...
    expect(message).toMatchInlineSnapshot(`
      ...
      If there are internal otto issues (sessions not responding, bot errors, unexpected behavior), read the log file at \`/tmp/otto-test-kqTlkv/otto.log\`. This file contains detailed logs of all bot activity including session creation, event handling, errors, and API calls. The log file is reset every time the bot restarts, so it only contains logs from the current run.

      ## uploading files to discord

      To upload files to the Discord thread (images, screenshots, long files that would clutter the chat), run:

      otto upload-to-discord --session ses_123 <file1> [file2] ...
      ...
      otto session archive --session ses_123
      ...
      otto user list --guild guild_123 --query "username"
      ...
      otto send --channel chan_123 --prompt "your prompt here" --agent <current_agent> --user "Tommy"
      ...
      otto send --channel chan_123 --prompt "User cancelled subscription" --notify-only --agent <current_agent> --user "Tommy"

      Use --user to add a specific Discord user to the new thread:

      otto send --channel chan_123 --prompt "Review the latest CI failure" --agent <current_agent> --user "Tommy"

      Use --worktree to create a git worktree for the session (ONLY when the user explicitly asks for a worktree):

      otto send --channel chan_123 --prompt "Add dark mode support" --worktree dark-mode --agent <current_agent> --user "Tommy"

      Use --cwd to start a session in an existing git worktree directory (must be a worktree of the project):

      otto send --channel chan_123 --prompt "Continue work on feature" --cwd /path/to/existing-worktree --agent <current_agent> --user "Tommy"
      ...
      otto send --channel chan_123 --prompt "Plan the refactor of the auth module" --agent plan --user "Tommy"
      ...
      otto send --channel chan_123 --prompt "/build-cmd update dependencies" --agent <current_agent> --user "Tommy"
      ...
      otto send --channel chan_123 --prompt "Reminder: review open PRs" --send-at "2026-03-01T09:00:00Z" --agent <current_agent> --user "Tommy"
      otto send --channel chan_123 --prompt "Run weekly test suite and summarize failures" --send-at "0 9 * * 1" --agent <current_agent> --user "Tommy"
      ...
      - Example no-op cleanup command: \`otto session archive --session ses_123\`
      ...
      - Proactive reminders: when you encounter time-sensitive information during your work (e.g. creating an API key that expires in 90 days, a certificate with an expiration date, a trial period ending, a deadline mentioned in code comments), proactively schedule a \`--notify-only\` reminder before the expiration so the user gets notified in time. For example, if you generate an API key expiring on 2026-06-01, schedule a reminder a few days before: \`otto send --channel chan_123 --prompt "Reminder: <@USER_ID> the API key created on 2026-03-01 expires on 2026-06-01. Renew it before it breaks production." --send-at "2026-05-28T09:00:00Z" --notify-only --agent <current_agent>\`. Always tell the user you scheduled the reminder so they know.
      ...
      - Quiet no-op checks: if a recurring task checks something and finds nothing to report, let it post a brief final summary and then archive the session with \`otto session archive --session ses_123\`. Example: a scheduled email triage run that finds no new emails should archive itself so it does not add noise to Discord.
      ...
      otto send --session ses_123 --prompt "Reminder: <@USER_ID> you asked to be reminded about this thread." --send-at "<future_UTC_time>" --notify-only --agent <current_agent>
      ...
      \`\`\`bash
      otto send --channel chan_123 --prompt "your task description" --worktree worktree-name --agent <current_agent> --user "Tommy"
      \`\`\`
      ...
      \`\`\`bash
      otto send --channel chan_123 --prompt "Continue work on feature X" --cwd /path/to/existing-worktree --agent <current_agent> --user "Tommy"
      \`\`\`
      ...
      **Important:** When using \`otto send\`, prefer combining investigation and action into a single session instead of splitting them. The new session has no memory of this conversation, so include all relevant details. Use **bold**, \`code\`, lists, and > quotes for readability.
      ...
      \`\`\`bash
      otto send --channel chan_123 --prompt "Continuing from previous session: <summary of current task and state>" --agent <current_agent> --user "Tommy"
      \`\`\`
