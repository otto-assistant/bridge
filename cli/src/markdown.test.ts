    ...
    then: {
      parts: [
        ...
        { type: "text-start", id: "hello-text" },
        {
          type: "text-delta",
          id: "hello-text",
          delta: "Hello! This is a deterministic markdown test response.",
        },
...
function normalizeMarkdown(md: string): string {
  return (
    md
      // Drop completion timing line to avoid environment-dependent footer jitter
      .replace(/\n\*Completed in [\d.]+[ms]+\*\n?/g, "\n")
...
test("error handling for non-existent session", async () => {
  ...
  expect((result as Error).message).toContain(
    `Session ${badSessionID} not found`,
  );
});
