...
const serverLifecycleListeners = new Set<
  (event: ServerLifecycleEvent) => void
>();
...
function ensureProcessCleanupHandlersRegistered(): void {
  ...
  // In agent-first flows (e.g. send --silent-prompt), the CLI process bootstraps
  // a session and exits while the bot continues streaming it. Killing the shared
  // opencode server on CLI exit would drop the in-flight run.
  if (process.env.KIMAKI_SKIP_OPENCODE_PROCESS_CLEANUP === "1") {
    opencodeLogger.log("Skipping opencode process-exit cleanup handlers");
    return;
  }
  ...
  opencodeLogger.log(
    "Registering process cleanup handlers for opencode server",
  );
...
export function resolveOpencodeCommand(): string {
  ...
  if (result instanceof Error) {
    ...
    opencodeLogger.warn(
      'Could not resolve opencode path via which, falling back to "opencode"',
    );
    return "opencode";
  }
...
async function startSingleServer(): Promise<ServerStartError | SingleServer> {
  ...
  const kimakiShimDirectory = ensureKimakiCommandShim({
    ...
    entryScript:
      process.argv[1] || fileURLToPath(new URL("../bin.js", import.meta.url)),
  });
...
export function parsePermissionRules(raw: unknown): PermissionRuleset {
  ...
  return raw.flatMap((entry) => {
    ...
    if (parts.length >= 3) {
      ...
      return [
        { permission, pattern, action: action as "allow" | "deny" | "ask" },
      ];
    }
    return [];
  });
 ...
 */
export function removeInjectionGuardConfig({
  sessionId,
}: {
  sessionId: string;
}): void {
