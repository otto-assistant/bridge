...
export async function startDiscordBot({
      ...
      if (channel.type === ChannelType.GuildText && !isCliInjectedPrompt) {
        ...
        if (mentionModeEnabled) {
          ...
          if (!botMentioned && !isShellCommand) {
            voiceLogger.log(
              `[IGNORED] Mention mode enabled, bot not mentioned`,
            );
            return;
          }
        }
      ...
      if (isThread) {
        ...
        if (enqueueResult.queued && enqueueResult.position) {
          await sendThreadMessage(
            thread,
            `Queued at position ${enqueueResult.position}`,
          );
        }
      }
  ...
  discordClient.on(Events.ThreadCreate, async (thread, newlyCreated) => {
    try {
      ...
      if (marker.worktree) {
        discordLogger.log(
          `[BOT_SESSION] Creating worktree: ${marker.worktree}`,
        );
      ...
      if (marker.cwd) {
        ...
        if (cwdResult instanceof Error) {
          discordLogger.error(
            `[BOT_SESSION] --cwd validation failed: ${cwdResult.message}`,
          );
          await thread.send({
  ...
  startHeapMonitor();
  const stopTaskRunner = startTaskRunner({ token, discordClient });
